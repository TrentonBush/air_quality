...
from .i2c_base import Device, Register, Field, Encoder, LookupTable


class SignedInt16(Encoder):
    @staticmethod
...
class BMP280(object):
    def __init(self, i2c_device: SMBus, address_pin_level: int = 0):
        ...
        self.address = _PROPERTIES.i2c_addresses[self._address_level]
        self._cached = {
            register.name: {field.name: None for field in register.fields.values()}
            for register in _PROPERTIES.registers.values()
        }
